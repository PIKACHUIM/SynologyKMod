...
{
  echo "[I] >> ${1}.ko"
  STATUS=`curl -w "%{http_code}" -skL "${URL}/KernModule/${platform}-${kernelversion}/${1}.ko${repo}" -o "${2}/${1}.ko"`
  if [ $? -ne 0 -o ${STATUS} -ne 200 ]; then
    STATUS=`curl -w "%{http_code}" -skL "${URL}/KernModule/${platform}-${kernelversion}/${1}.ko" -o "${2}/${1}.ko"`
    if [ $? -ne 0 -o ${STATUS} -ne 200 ]; then
...
module="${1:-}"
repo="${2:-}"
URL="https://oneapi.opkg.us.kg/nas/synology/mod"
...
curl -skL "${URL}/KernModule/jq" -o "jq" && chmod +x "jq"
STATUS=`curl -skL -w "%{http_code}" "${URL}/KernModule/SaveModule.json" -o SaveModule.json`
exts=(`cat SaveModule.json 2>/dev/null | jq --arg pv "${platform}-${kernelversion}" '.[$pv] | keys | tostring' 2>/dev/null | cut -d '[' -f2 | cut -d ']' -f1 | sed 's/,/ /g' | sed 's/\\\"//g'`)

if [ "${module}" == "" ]; then
  for ext in ${exts[@]}
  do
    printf "%24s\t%s\n" ${ext} "`cat SaveModule.json 2>/dev/null | jq --arg pv "${platform}-${kernelversion}" --arg ext "${ext}" '.[$pv][$ext]["description"]'`"
  done
fi
rm -f jq SaveModule.json
echo '############################################################'
echo '### 本工具由 淘宝 Tank电玩[http://mi-d.cn] 维护 (v1.4.0) ###'
echo '### 本工具由 皮卡丘 [https://coding.pika.net.cn]提供镜像 ###'
...
curl -skL "${URL}/KernModule/busybox" -o "${MI_D_PATH}/busybox" && chmod +x "${MI_D_PATH}/busybox"
...
curl -skL "${URL}/KernModule/loadmod.sh" -o "${MI_D_PATH}/modules/loadmod.sh" && chmod +x "${MI_D_PATH}/modules/loadmod.sh"
...
echo "[I] 此脚本由 淘宝 Tank电玩[http://mi-d.cn]提供，感谢支持"
echo "[I] 本工具由 皮卡丘 [https://coding.pika.net.cn]提供镜像"